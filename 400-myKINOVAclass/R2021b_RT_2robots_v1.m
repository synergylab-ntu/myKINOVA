...
u = udpport("IPV4","LocalHost","127.0.0.1",'LocalPort',RECV_PORT)

u2 = udp
u2.RemotePort = SEND_PORT
fopen(u2)

u4 = udp
u4.RemotePort = SEND_PORT2
fopen(u4)
...
while curr_time < ROBOT.t(end) - 2 % VERY IMPORTANT - keep interpolation within limits
    curr_time = toc(timer1);
    curr_unix_epoch = datenum2unixmillis(now);
    des_q = transpose(interp1(ROBOT.t,ROBOT.q',curr_time));
    MATLAB_RT(i).des_q = des_q;
    MATLAB_RT(i).curr_unix_epoch = curr_unix_epoch;
    MATLAB_RT(i).curr_time = curr_time;
...
save(data_filename,'MATLAB_RT','ROBOT');